...
class KafkaConfiguration(
    ...
    fun topics(): NewTopics {
        val topics = kafkaTopicProperties.values.flatMap {
            ...
            val deadLetterTopic = TopicBuilder
                .name("${it.name}${com.leijendary.spring.template.websocket.core.config.TOPIC_DEAD_LETTER_SUFFIX}")
                .partitions(1)
                .replicas(1)
                .build()
    ...
    fun kafkaListenerContainerFactory(
        ...
        val recover = DeadLetterPublishingRecoverer(template) { record, _ ->
            TopicPartition(
                record.topic() + com.leijendary.spring.template.websocket.core.config.TOPIC_DEAD_LETTER_SUFFIX,
                record.partition()
            )
        }
